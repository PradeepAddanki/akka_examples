...
public class WebSocketClientExampleTest {
    ...
    public void testSingleWebSocketRequest() {
        ...
        final Pair<CompletionStage<WebSocketUpgradeResponse>, CompletionStage<Done>> pair =
                http.singleWebSocketRequest(
                        WebSocketRequest.create("ws://localhost:9001/stats"),
                        flow,
                        materializer
                );
    ...
    public void  testWebSocketClientFlow(String url){
        ActorSystem system = ActorSystem.create();
        Materializer materializer = ActorMaterializer.create(system);
        Http http = Http.get(system);
...
// send this as a message over the WebSocket
        ...
        Flow<Message, Message, CompletionStage<WebSocketUpgradeResponse>> webSocketFlow =
                http.webSocketClientFlow(WebSocketRequest.create(url));
...
// and handle errors more carefully
    ...
    public void halfClosedWebSocketClosingExample() {
        ...
        http.singleWebSocketRequest(
                WebSocketRequest.create("ws://localhost:8080/greeter"),
                flow,
                materializer);
    ...
    public void halfClosedWebSocketFiniteWorkingExample(String url) {
        final ActorSystem system = ActorSystem.create();
        final Materializer materializer = ActorMaterializer.create(system);
        final Http http = Http.get(system);
...
//    }
    ...
    public static void main(String[] args) {
        WebSocketClientExampleTest test = new WebSocketClientExampleTest();
        test.testSingleWebSocketRequest();
        //test.halfClosedWebSocketFiniteWorkingExample("ws://localhost:9001/stats");
